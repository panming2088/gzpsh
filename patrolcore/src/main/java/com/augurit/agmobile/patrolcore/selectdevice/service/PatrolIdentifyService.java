 ...
 * @author 创建人 ：xuciluan
 * @version 1.0
 * @package 包名 ：com.augurit.agmobile.agpatrol.identify
 * @createTime 创建时间 ：17/7/24
 * @modifyBy 修改人 ：xuciluan
 * @modifyTime 修改时间 ：17/7/24
 * @modifyMemo 修改备注：
 */

public class PatrolIdentifyService implements IIdentifyService {
        ...
        getQueryableLayerMap(context, visibleQueryableLayers)
                .map(new Func1<Map<String, int[]>, Map<IdentifyTask, IdentifyParameters>>() {
                    ...
                    public Map<IdentifyTask, IdentifyParameters> call(Map<String, int[]> layers) {
                        //封装Identify条件
                        return PatrolIdentifyService.this.getIdentifyParametersMap(layers, geometry, mapView, tolerance);
                    }
                })
                ...
                .map(new Func1<List<IdentifyResult>, IdentifyResult[]>() {
                    ...
                    public IdentifyResult[] call(List<IdentifyResult> results) {
                        //将结果根据点/线/面的顺序进行排序
                        return PatrolIdentifyService.this.sortedIdentifyResult(results);
                    }
                })
                //进行过滤掉相同的选项
