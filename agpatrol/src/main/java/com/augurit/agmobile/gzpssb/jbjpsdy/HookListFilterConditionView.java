...
public class HookListFilterConditionView {
    ...
    private String[] jhjlx = {"全部", "接驳井", "接户井"};
    ...
    public View initView(String filterTitle) {
         ...
         * 窨井类型
         */
        final FlexRadioGroup frg_facility_type0 = (FlexRadioGroup) root.findViewById(R.id.frg_facility_type0);
        createYjTypeRadioButtons(frg_facility_type0);
        ...
        root.findViewById(R.id.btn_submit).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String type0 = null;
                RadioButton rbFacilityTYpe0 = (RadioButton) root.findViewById(frg_facility_type0.getCheckedRadioButtonId());
                if (rbFacilityTYpe0 != null) {
                    type0 = rbFacilityTYpe0.getText().toString();
                }
...
//                    }
     ...
     * @param group
     */
    private void createYjTypeRadioButtons(final FlexRadioGroup group) {
        float margin = DensityUtils.dp2px(mContext, 60);
        float width = DensityUtils.getWidth(mContext);
        for (String filter : jhjlx) {
            final RadioButton rb = (RadioButton) View.inflate(mContext, R.layout.item_radiobutton, null);
            rb.setText(filter);
            FlexboxLayout.LayoutParams lp = new FlexboxLayout.LayoutParams((int) (width - margin) / 4, ViewGroup.LayoutParams.WRAP_CONTENT);
            rb.setLayoutParams(lp);
            group.addView(rb);
            if (filter.equals("全部") || jhjlx.length == 1) {
                rb.setChecked(true);
            }
