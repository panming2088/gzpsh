...
public class PshSendNextLinkActivity extends BaseActivity {
    ...
    private PSHEventDetail.NextlinkBean selectedNextlinkBean;    //当前选中的下一环节
                ...
                .subscribe(new Action1<Void>() {
                    ...
                    public void call(Void aVoid) {
                        if (selectedNextlinkBean == null) {
                            ToastUtil.shortToast(PshSendNextLinkActivity.this, "请选择下一环节");
                            return;
                        }
//                        if(GzpsConstant.LINK_SEND_TASK.equals(currLinkCode)
...
//                                    || selAssignee == null){
//                                ToastUtil.shortToast(PshSendNextLinkActivity.this, "请选择下一环节处理人");
//                                return;
//                            }
//                        }
//                        if(GzpsConstant.LINK_PROBLEM_REPORT.equals(currLinkCode)
...
//                        }
                        if(isZhifa && sfzf == null){
                            ToastUtil.shortToast(PshSendNextLinkActivity.this, "请选择是否执法！");
                            return;
                        }

                        List<Photo> photos = photo_item.getSelectedPhotos();
                        if(takePhoto && (ListUtil.isEmpty(photos) ||  photos.size()<2)){
                            ToastUtil.shortToast(PshSendNextLinkActivity.this, "请至少提供两张照片！");
                            return;
                        }
                        if (isContent && StringUtil.isEmpty(et_content.getText().toString())) {
                            if(!StringUtil.isEmpty(name)){
                                ToastUtil.shortToast(PshSendNextLinkActivity.this, "请填写"+name);
                            }else {
                                ToastUtil.shortToast(PshSendNextLinkActivity.this, "请填写意见");
                            }
                            return;
                        }
                        ...
                        eventService.wfSend(wtsbId,
                                ...
                                .subscribe(new Subscriber<Boolean>() {
                                    ...
                                    public void onError(Throwable throwable) {
                                        pd.dismiss();
                                        ToastUtil.shortToast(PshSendNextLinkActivity.this, "操作失败，请重试！");
                                    }
                                    ...
                                    public void onNext(Boolean s) {
                                        ...
                                        setResult(456);
                                        finish();
                                        ToastUtil.shortToast(PshSendNextLinkActivity.this, "操作成功");
...
//                        radio_nextlink_org.check(0);
    ...
    private void getNextLinkAssigneers() {
        new UploadEventService(getApplicationContext()).getNextActivityInfo(taskInstId)
                ...
                .subscribe(new Subscriber<Result<List<Assigneers>>>() {
                    ...
                    public void onError(Throwable throwable) {
                        pd.dismiss();
                        ToastUtil.shortToast(PshSendNextLinkActivity.this, "获取下一环节处理人失败");
                    }
    ...
    private void saveJdFile() {
        ...
        uploadEventService.saveJdFile(taskInstId, photo_item.getSelectedPhotos())
                ...
                .subscribe(new Subscriber<Boolean>() {
                    ...
                    public void onError(Throwable throwable) {
                        pd.dismiss();
                        ToastUtil.longToast(PshSendNextLinkActivity.this, "提交下一环节成功，但上传附件失败！");
                    }
                    ...
                    public void onNext(Boolean aBoolean) {
                        ...
                        if (aBoolean) {
                            ...
                            ToastUtil.shortToast(PshSendNextLinkActivity.this, "操作失败！");
                        }
    ...
    public void onBackPressed() {
        DialogUtil.MessageBox(PshSendNextLinkActivity.this, "提示", "是否确定放弃本次编辑？", new DialogInterface.OnClickListener() {
            @Override
