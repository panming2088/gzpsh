...
public class AppInstallDetailActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(curPos == 0 || curPos == 1) {
            ...
            if (TextUtils.isEmpty(isInstallStr)) {
                //柱状图点击
                String url = "http://" + LoginConstant.GZPSH_AGSUPPORT +
                        "/rest/installPsh/StatisticalAppInOrg?org_name=" + org_name + "&roleType="
                        + roleType;
                getAppInstallInfo(url);
            } else{
                //安装数字点就点击
                boolean isInstalled = Boolean.parseBoolean(isInstallStr);
                String url = "http://" + LoginConstant.GZPSH_AGSUPPORT +
                        "/rest/installPsh/StatisticalAppGetUsers?org_name=" +
                        org_name + "&isInstalled=" + isInstalled + "&roleType=" + roleType;
                getAppInstallInfo(url);
            }
        }else if(curPos == 2){
            if (TextUtils.isEmpty(isInstallStr)) {
                //柱状图点击
                String url ="http://" + LoginConstant.GZWS_AGSUPPORT  +
                        "rest/installRecord/StatisticalAppNwInOrg?org_name=" + org_name + "&roleType="
                        + roleType;
                getAppInstallInfo(url);
            } else{
                //安装数字点就点击
                boolean isInstalled = Boolean.parseBoolean(isInstallStr);
                String url ="http://" + LoginConstant.GZWS_AGSUPPORT  +
                        "rest/installRecord/StatisticalNwAppGetUsers?org_name=" +
                        org_name + "&isInstalled=" + isInstalled + "&roleType=" + roleType;
                getAppInstallInfo(url);
            }
        }
    ...
    private void setViewData(ArrayList<InstallInfo.InstallUser> installUsers) {
        ...
        MyAdapter myAdapter = new MyAdapter(installUsers, this);
