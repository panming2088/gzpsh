package com.augurit.agmobile.mapengine.bufferanalysis.util;
...
@Keep
public class BufferMapOnTouchListener extends MapOnTouchListener {
    ...
    public BufferMapOnTouchListener(Context context, MapView view,GraphicsLayer graphicsLayer) {
        super(context, view);
        mContext = context;
        mMapView = view;
        init(graphicsLayer);
    ...
    private Symbol getDefaultSymbol(Geometry.Type type){
        Symbol symbol = null;
        SimpleLineSymbol lineSymbol = new SimpleLineSymbol(Color.parseColor("#2196F3"),
    ...
    public void addGeometry(Geometry geometry){
        ...
        mCurrentGraphicID = mGLayer.addGraphic(graphic);
        mGeometry = geometry;
    }
     ...
     * @return 缓冲后的区域
     */
    public Geometry buffer(double radius,SpatialReference spatialReference){
        if(mGeometry == null
                || mCurrentGraphicID == -1){
            return null;
        }
        ...
        try {
            //生成缓冲区
            Geometry bufferGeometry = GeometryEngine.buffer(mGeometry, spatialReference,
                    radius, null);
            FillSymbol fillSymbol = new SimpleFillSymbol(Color.parseColor("#2196F3"));
