package com.augurit.agmobile.mapengine.common.baidutransform.pointer.base;
        ...
        if (other == this) {
            ...
            if (other instanceof GeoPointer) {
                ...
                return df.format(latitude).equals(df.format(otherPointer.latitude))
                        && df.format(longtitude).equals(df.format(otherPointer.longtitude));
            } else {
                return false;
            }
        }
    ...
    public double distance(GeoPointer target) {
        ...
        double x =
                Math.cos(this.latitude * Math.PI / 180) * Math.cos(target.latitude * Math.PI / 180)
                        * Math.cos((this.longtitude - target.longtitude) * Math.PI / 180);
