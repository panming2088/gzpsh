...
public final class ObjectMapUtil {
    ...
    public static Map<String, String> getFieldValue(Object obj)
        ...
        for (Field field : fields) {
            ...
            if ("boolean".equals(className.toLowerCase())) {
                if (name.startsWith("is")) {
                    if (name.length() < 3) {
                        strGet = "is" + name.substring(0, 1).toUpperCase()
                                + name.substring(1, name.length());
                    } else {
                        String firstName = name.substring(2, 3);
                ...
                strGet = "get" + name.substring(0, 1).toUpperCase()
                        + name.substring(1, name.length());
            }
            Method methodGet = cls.getDeclaredMethod(strGet);
            Object object = methodGet.invoke(obj);
            String valueString = JsonUtil.getJson(object == null ? "" : object);
            if (valueString.startsWith("\"")) {
                valueString = valueString
                        .substring(1, valueString.length() - 1);
            }
            mapValue.put(name, valueString);
        }
        return mapValue;
    }
