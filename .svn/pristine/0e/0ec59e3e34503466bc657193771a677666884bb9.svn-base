...
/**
 * 跟{@link AlphaTabView}的区别在于padding的不同,默认是5；
 * @author 创建人 ：xuciluan
 * @version 1.0
 * @package 包名 ：com.augurit.agmobile.agpatrol.widget
 * @createTime 创建时间 ：17/8/18
 * @modifyBy 修改人 ：xuciluan
 * @modifyTime 修改时间 ：17/8/18
 * @modifyMemo 修改备注：
 */

public class PatrolAlphaTagView extends View {
    ...
    private int mPadding = 4;                      //文字和图片之间的距离 5dp
    ...
    public PatrolAlphaTagView(Context context) {
        this(context, null);
        mContext = context;
    }

    public PatrolAlphaTagView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
        mContext = context;
    }

    public PatrolAlphaTagView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void drawBadge(Canvas canvas) {
        int i = getMeasuredWidth() / 12;
        int j = getMeasuredHeight() / 8;
        i = i >= j ? j : i;
        if (mBadgeNumber > 0) {
