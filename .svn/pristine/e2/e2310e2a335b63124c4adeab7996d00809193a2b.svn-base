 ...
 * 默认离线采用geodatabase和shapefile结合的方式，在线采用agcom接口；
 * @author 创建人 ：xuciluan
 * @version 1.0
...
public class CombinedOfflineAndAgcomLayerQueryService extends AgcomLayerQueryService {


    public CombinedOfflineAndAgcomLayerQueryService(Context context) {
        super(context);
    }
    ...
    public void queryLayer(Context context, String seachKey,
        ...
        if (NetUtil.isConnected(context) && !ValidateUtil.isListNull(onlineLayer)){ //如果网络可达
            AgcomLayerQueryService layerQueryService = new AgcomLayerQueryService(context);
    ...
    public Observable<List<LayerInfo>> getQueryableLayers(Context context) throws IOException {
        if (NetUtil.isConnected(context)){ //如果网络通畅
            return new AgcomLayerQueryService(context).getQueryableLayers(context);
        }
