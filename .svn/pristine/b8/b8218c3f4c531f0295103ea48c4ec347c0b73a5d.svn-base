...
import com.augurit.agmobile.gzpssb.fire.adapter.MyUploadedFireListAdapter;
...
public class PSHFireListFragment extends Fragment {

    private MyUploadedFireListAdapter mSearchResultAdapter;
    ...
    private UploadFireFilterConditionEvent mFireEvent;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        View inflate = inflater.inflate(R.layout.fragment_my_add_fire, null);
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        mSearchResultAdapter = new MyUploadedFireListAdapter(mContext, new ArrayList<GroundfireBean>());
        ...
        mSearchResultAdapter.setOnItemClickListener(new BaseRecyclerAdapter.OnItemClickListener() {
            ...
            public void onItemClick(View view, int position, long id) {
                ...
                GroundfireBean groundfireBean  = mSearchResultAdapter.getDataList().get(position - 1);
                groundfireBean.setPhotos8(Attachment2PhotoUtil.toPhotos(groundfireBean.getHasCert8Attachment()));
                intent.putExtra("groundfireBean", groundfireBean);
    ...
    public void loadDatas(boolean ifShowPb) {
        ...
        final PSHUploadFireService pshUploadFireService = new PSHUploadFireService(mContext);
        boolean isFilter = null != mFireEvent;
        Calendar calendar = Calendar.getInstance();
        pshUploadFireService.getFireList(page,isFilter ? mFireEvent.getArea():"",
                isFilter ? mFireEvent.getSfwh():"",isFilter ? mFireEvent.getSfls():"",
                isFilter ? mFireEvent.getSzwz():"", isFilter ?mFireEvent.getAddr():"",
                isFilter ? mFireEvent.getName():"",
                isFilter ? mFireEvent.getStartTime():new Date(calendar.get(Calendar.YEAR) - 1900, 0, 1).getTime(),
                isFilter ? mFireEvent.getEndTime():new Date(calendar.get(Calendar.YEAR) - 1900, calendar.get(Calendar.MONTH) , calendar.get(Calendar.DAY_OF_MONTH) + 1).getTime(),
                isFilter ? mFireEvent.getUploadid():null)

                .map(new Func1<Result3<List<GroundfireBean>>, List<GroundfireBean>>() {
                    @Override
                    public List<GroundfireBean> call(final Result3<List<GroundfireBean>> listResult2) {
                ...
                .subscribe(new Subscriber<List<GroundfireBean>>() {
                    @Override
...
//                                EventBus.getDefault().post(new UploadFacilitySumEvent(total));
                    ...
                    @Override
                    public void onNext(List<GroundfireBean> beanList) {
    ...
    @Subscribe
    public void refreshList(UploadFireFilterConditionEvent fireFilterCondition) {
            page = 1;
            this.mFireEvent = new UploadFireFilterConditionEvent(fireFilterCondition.getArea(),
                    fireFilterCondition.getSfwh(),fireFilterCondition.getSfls(),fireFilterCondition.getSzwz(),
                    fireFilterCondition.getAddr(),fireFilterCondition.getName(),fireFilterCondition.getStartTime(),
                    fireFilterCondition.getEndTime(),fireFilterCondition.getUploadid());
            loadDatas(true);
    }
    @Subscribe
