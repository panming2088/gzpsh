...
public class EasyPermissions {
    ...
    public static String[] checkPermissions(Context context, @NonNull String[] perms){
        // Always return true for SDK < M, let the system deal with the permissions
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            return perms;
        }
        ...
        List<String> checkedPerms = new ArrayList<>();
        ...
        return (String[]) checkedPerms.toArray();
    }

    public static int[] checkPermissions2(Context context, @NonNull String[] perms){
        ...
        for (int j=0; j<perms.length; j++) {
            ...
            if (ContextCompat.checkSelfPermission(context, perm)
                    != PackageManager.PERMISSION_GRANTED) {
                grantResults[j] = PackageManager.PERMISSION_DENIED;
            }
        }
        return grantResults;
    }
    ...
    private static void requestPermissions(
        ...
        if (hasPermissions(helper.getContext(), perms)) {
            notifyAlreadyHasPermissions(helper.getHost(), perms);
            return;
        }
        ...
        for (Object object : receivers) {
            ...
            if (!granted.isEmpty()) {
                if (object instanceof PermissionCallbacks) {
                    ((PermissionCallbacks) object).onPermissionsGranted(granted);
                }
            }
            ...
            if (!denied.isEmpty()) {
                if (object instanceof PermissionCallbacks) {
                    ((PermissionCallbacks) object).onPermissionsDenied(denied);
                }
            }

            // If 100% successful, call annotated methods
            if (!granted.isEmpty() && denied.isEmpty()) {
                runAnnotatedMethods(object);
            }
        }
    }
     ...
     *                          PermissionCallbacks#onPermissionsDenied(List)}
    ...
    public static boolean somePermissionPermanentlyDenied(@NonNull Activity host,
        ...
        return PermissionHelper.newInstance(host)
                .somePermissionPermanentlyDenied(requestCode, deniedPermissions);
    }

    /**
     * @see #somePermissionPermanentlyDenied(Activity, int, List)
     */
    ...
    public static boolean permissionPermanentlyDenied(@NonNull Activity host,
        ...
        return PermissionHelper.newInstance(host).permissionPermanentlyDenied(requestCode, deniedPermission);
    }

    /**
     * @see #permissionPermanentlyDenied(Activity, int, String)
     */
    ...
    public static boolean somePermissionDenied(@NonNull Activity host,
        ...
        return PermissionHelper.newInstance(host).somePermissionDenied(requestCode, perms);
    }

    /**
     * @see #somePermissionDenied(Activity, int, String...)
     */
    ...
    private static void notifyAlreadyHasPermissions(@NonNull Object object,
        ...
        onRequestPermissionsResult(perms, grantResults, object);
    }
     ...
     */
    private static void runAnnotatedMethods(@NonNull Object object) {
