...
/**
 * 以下Bottomsheet实现的功能是：
 * （1）当内部的控件是可滑动时，优先响应内部滑动控件的事件，只有当手指触摸到不可滑动的
 * 部分时，才可以拖动bottomsheet；
 ...
 * @version 1.0
 */
public class BottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> implements IBottomSheetBehavior{
    ...
    *//** @hide *//*
    ...
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, V child, View target, int dx,
        ...
        if (target == scrollingChild) { //如果是滚动部分
            if (dy < 0 &&  target.getScrollY() == 0 && mState == STATE_EXPANDED){
    ...
    public void onStopNestedScroll(CoordinatorLayout coordinatorLayout, V child, View target) {
        ...
        if (mLastNestedScrollDy > 0) {
            ...
            /*if(childTop > anchorPeekMidOffect){
                top = mMaxOffset;
                targetState = STATE_COLLAPSED;
            } else if(childTop < anchorPeekMidOffect
                    && childTop > anchorMinMidOffect){
                top = anchorOffect;
                targetState = STATE_ANCHOR;
            } else {
                top = mMinOffset;
